...
public class AlipaySubmit {
	...
	public static String buildRequestMysign(Map<String, String> sPara) {
		...
		String mysign = MD5.sign(prestr, AlipayWebConfig.ALIPAY_KEY, AlipayWebConfig.input_charset);
		return mysign;
	}
	...
	public static String buildRequest(Map<String, String> sParaTemp, String strMethod, String strButtonName, String strParaFileName) {
		...
		StringBuffer sbHtml = new StringBuffer();
		sbHtml.append("<form id=\"alipaysubmit\" name=\"alipaysubmit\"  enctype=\"multipart/form-data\" action=\"" + ALIPAY_GATEWAY_NEW + "_input_charset=" + AlipayWebConfig.input_charset
				+ "\" method=\"" + strMethod + "\">");
	...
	public static String buildRequest(String strParaFileName, String strFilePath, Map<String, String> sParaTemp) throws Exception {
		...
		request.setCharset(AlipayWebConfig.input_charset); // 设置编码集
		request.setParameters(generatNameValuePair(sPara));
		request.setUrl(ALIPAY_GATEWAY_NEW + "_input_charset=" + AlipayWebConfig.input_charset);

		HttpResponse response = HttpProtocolHandler.getInstance().execute(request, strParaFileName, strFilePath);
		if (response == null) {
			return null;
		}
	...
	public static String query_timestamp() throws MalformedURLException, DocumentException, IOException {
		...
		String strUrl = ALIPAY_GATEWAY_NEW + "service=query_timestamp&partner=" + AlipayWebConfig.ALIPAY_PARTNER + "&_input_charset" + AlipayWebConfig.input_charset;
	...
	public static String buildForm(Map<String, String> sParaTemp, String gateway, String strMethod, String strButtonName) {
		...
		StringBuffer sbHtml = new StringBuffer();
		sbHtml.append("<form id=\"alipaysubmit\" name=\"alipaysubmit\" enctype=\"multipart/form-data\" action=\"" + gateway + "_input_charset=" + AlipayWebConfig.input_charset + "\" method=\""
				+ strMethod + "\">");
		...
		sbHtml.append("<input type=\"file\" name=\"bptb_pay_file\">");
		// submit按钮控件请不要含有name属性
