...
/**
 * 统一下单 {@link https://api.mch.weixin.qq.com/pay/unifiedorder}
 * 
 * @author Jack
 */
public class UnifiedOrderRequestBuilder extends RequestBuilder {
	...
	/** 交易起始时间 String(14) 订单生成时间，格式为yyyyMMddHHmmss，如2009年12月25日9点10分10秒表示为20091225091010。 */
	private String time_start = "";
	/** 交易结束时间 String(14) 订单失效时间，格式为yyyyMMddHHmmss，如2009年12月27日9点10分10秒表示为20091227091010。注意：最短失效时间间隔必须大于5分钟 */
	private String time_expire = "";
	/** 商品标记 String(32) 商品标记，使用代金券或立减优惠功能时需要的参数，说明详见代金券或立减优惠 */
	private String goods_tag = "";
	/** [必填]通知地址 String(256) 异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数。 */
	private String notify_url = "";
	/** [必填]交易类型 String(16) 取值如下：JSAPI，NATIVE，APP等，说明详见参数规定 */
	...
	/** 用户标识 String(128) trade_type=JSAPI时（即公众号支付），此参数必传，此参数为微信用户在商户对应appid下的唯一标识。openid如何获取，可参考【获取openid】。企业号请使用【企业号OAuth2.0接口】获取企业号内成员userid，再调用【企业号userid转openid接口】进行转换 */
	private String openid = "";
	/**该字段用于上报支付的场景信息,针对H5支付有以下三种场景,请根据对应场景上报,H5支付不建议在APP端使用，针对场景1，2请接入APP支付，不然可能会出现兼容性问题*/
	private String scene_info = "";

	public static void main(String[] args) {
		UnifiedOrderRequestBuilder b = new UnifiedOrderRequestBuilder();
		...
		b.setNotify_url("WxpayAsynReceivedAction!received.action");
		b.setOut_trade_no("299920170419135918");
		b.setProduct_id("13844");
		b.setSpbill_create_ip("166.111.3.242");
		b.setTime_start("20170419164614");
		b.setTime_expire("20170419184614");
		b.setTotal_fee("1");
		b.setTrade_type("NATIVE");
	...
	public String buildDigest() {
		...
		param.put("time_start", time_start); // 交易起始时间，订单生成时间，格式为yyyyMMddHHmmss，如2009年12月25日9点10分10秒表示为20091225091010。
		param.put("time_expire", time_expire); // 交易结束时间，订单失效时间，格式为yyyyMMddHHmmss，如2009年12月27日9点10分10秒表示为20091227091010。
		param.put("goods_tag", goods_tag); // 商品标记，使用代金券或立减优惠功能时需要的参数
		param.put("notify_url", notify_url);// 通知地址，必填，异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数。
		param.put("trade_type", trade_type);// 交易类型，必填，取值如下：JSAPI--公众号支付、NATIVE--原生扫码支付、APP--app支付、MICROPAY--刷卡支付
		param.put("product_id", product_id);// 商品ID，trade_type=NATIVE时（即扫码支付），此参数必传。此参数为二维码中包含的商品ID，商户自行定义。
		param.put("limit_pay", limit_pay); // 指定支付方式，上传此参数no_credit--可限制用户不能使用信用卡支付
		param.put("openid", openid); // 用户标识，trade_type=JSAPI时（即公众号支付），此参数必传，此参数为微信用户在商户对应appid下的唯一标识。
		param.put("scene_info", scene_info); // 该字段用于上报支付的场景信息,针对H5支付有以下三种场景,请根据对应场景上报
	...
	public String buildXmlString() {
		...
		if (StringUtils.isNotEmpty(time_start))
			sb.append("<time_start>" + time_start + "</time_start>");
